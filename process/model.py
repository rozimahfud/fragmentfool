...
class Net(nn.Module):
    ...
    def __init__(self, gated_graph_conv_args, conv_args, emb_size, dropout_rate, device):
    ...
    def forward(self, data):
        ...
        x = self.batch_norm(x)
        x = F.relu(x)
        x = F.dropout(x, p=self.dropout_rate, training=self.training)  # Reduce dropout rate
